    ...
    <div className="relative min-h-screen w-full bg-black text-white flex flex-col overflow-hidden">
      ...
      <div className="relative z-10 flex-1 max-w-md mx-auto w-full px-6 pt-8 pb-32">
        <header className="mb-8">
          ...
          <p className="text-[10px] opacity-30 uppercase tracking-[0.4em] font-bold mb-6">Complete missions to get coins</p>
          ...
          <div className="p-6 rounded-[32px] bg-zinc-900/50 border border-white/5 backdrop-blur-xl">
            ...
            <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden">
              <motion.div 
                ...
                className="h-full bg-gradient-to-r from-[#FFD700] to-[#FFA500] shadow-[0_0_10px_rgba(255,215,0,0.5)]"
              />
            </div>
          </div>
        </header>
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-10">
            {/* SOCIAL */}
            <section>
              <h3 className="text-[10px] font-black opacity-20 uppercase tracking-[0.5em] mb-4 px-2">Social Tasks</h3>
              <div className="flex flex-col gap-3">
                {social.map(task => (
                  <div key={task.id} className="p-4 rounded-[24px] bg-zinc-900/40 border border-white/5 flex items-center justify-between">
                    ...
                    {task.completion?.status === "completed" ? (
                      ...
                      <div className="flex gap-2">
                        {task.link && (
                          <a href={task.link} target="_blank" className="p-2.5 rounded-xl bg-white/5 border border-white/10">
            ...
            {/* SCREENSHOT */}
            <section>
              <h3 className="text-[10px] font-black opacity-20 uppercase tracking-[0.5em] mb-4 px-2">Proof Required</h3>
              <div className="flex flex-col gap-3">
                {screenshot.map(task => (
                  <div key={task.id} className="rounded-[28px] bg-zinc-900/40 border border-white/5 overflow-hidden">
                          ...
                          <div className="p-5 flex flex-col gap-4">
                            ...
                            {uploadUrl ? (
                              ...
                              <button 
                                ...
                                className="w-full py-10 border-2 border-dashed border-white/10 rounded-2xl flex flex-col items-center gap-2 hover:bg-white/5"
                              >
                                <Upload size={20} className="opacity-20" />
                                <span className="text-[10px] uppercase font-black opacity-20">Select Screenshot</span>
                              </button>
                            )}
                            <button 
                            ...
                            >
                              {submitting === task.id ? "Uploading..." : "Submit Proof"}
                            </button>
                          </div>
                        </motion.div>
                      )}
