...
interface CoinClickerProps {
  ...
  locked ? : boolean;
  needsAd ? : boolean;
  isAdVerified ? : boolean; // Interface penampung state ad eksternal
}
...
export default function CoinClicker({
  ...
  // Penentu mutlak mode koin emas aktif berdasarkan sisa waktu free atau status kelar nonton iklan
  const isCoinGoldActive = !locked && (!needsAd || isAdVerified);
  ...
  const handleClick = useCallback(
    ...
    [nextId, onCoin, locked, pointsPerClick, needsAd, isAdVerified]
  );
      ...
      >
        ...
        >
          ...
          <div className="absolute inset-2 rounded-full z-10" style={{ 
            background: (!isCoinGoldActive && needsAd)
          ...
          {/* =============================================================== */}
          {/* 🔥 INJEKSI LAYER EMAS UTAMA YANG AKURAT NYATA (ZETTA MATRIX REACTOR) */}
          {/* =============================================================== */}
          {isCoinGoldActive && (
          ...
          {/* ===== LAYER 5: INNER MEDALLION CORE UTAMA ===== */}
          <div
            className="relative w-[120px] h-[120px] rounded-full flex items-center justify-center z-30"
            style={{
              background: locked 
                ? "#333" 
                : (!isCoinGoldActive && needsAd)
                ...
                : "transparent", 
              boxShadow: (locked || (!isCoinGoldActive && needsAd)) ? "inset 0 4px 10px rgba(255,255,255,0.5), inset 0 -6px 12px rgba(39,39,42,0.6)" : "none",
              border: (locked || (!isCoinGoldActive && needsAd)) ? ((!isCoinGoldActive && needsAd) ? "2px solid rgba(113,113,122,0.55)" : "2px solid rgba(120,70,10,0.55)") : "none",
            }}
          >
            {(!isCoinGoldActive && needsAd) ? (
              ...
              // TAMPILAN UTAMA HURUF Z HITAM DI ATAS CENTER DISC
              <span className="font-black text-[38px] leading-none select-none text-black drop-shadow-[0_1px_0_rgba(255,255,255,0.4)]">
                Z
              </span>
            )}
          </div>
          ...
          {/* Specular highlight */}
          {!locked && <div className="absolute top-3 left-6 w-16 h-8 rounded-full pointer-events-none z-40" style={{ background: "radial-gradient(ellipse at center, rgba(255,255,255,0.85), rgba(255,255,255,0) 70%)" }} />}
        </motion.div>
      </motion.button>
    </div>
  );
