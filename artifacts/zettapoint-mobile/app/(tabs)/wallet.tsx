...
function validateTON(addr: string) {
  return /^[EeUu][Qq][A-Za-z0-9_\-]{46}$/.test(addr.trim());
}
function validateTRC20(addr: string) {
  return /^T[A-Za-z1-9]{33}$/.test(addr.trim());
}
function validateAddress(method: string, addr: string) {
...
export default function WalletScreen() {
  ...
  const handleConnectWallet = async () => {
    Alert.prompt(
      "Connect TON Wallet",
      "Enter your TON address (EQ... or UQ...)",
      async (input) => {
        if (!input) return;
        if (!validateTON(input)) {
          Alert.alert("Invalid address", "Must start with EQ or UQ, 48 characters total.");
          return;
        }
        const tid = await getUserId();
        if (!tid) return;
  ...
  const amountError =
    !amount ? "" :
    isNaN(amountNum) ? "Invalid amount" :
  ...
  return (
    <View style={[s.container, { paddingTop: topPad }]}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
        ...
        {/* TON Wallet connect */}
        <Pressable style={[s.tonRow, tonWalletAddress && s.tonRowConnected]} onPress={handleConnectWallet}>
          <Ionicons name="link" size={18} color={tonWalletAddress ? "#4CAF50" : colors.mutedForeground} />
          <View style={{ flex: 1 }}>
            <Text style={s.tonLabel}>TON WALLET</Text>
            <Text style={[s.tonAddr, tonWalletAddress && { color: "#4CAF50" }]}>
              {tonWalletAddress ? `${tonWalletAddress.slice(0, 10)}...${tonWalletAddress.slice(-6)}` : "Tap to connect"}
            </Text>
          </View>
        ...
        <View style={s.usdtCard}>
          ...
          <Pressable
            style={({ pressed }) => [s.wdBtn, pressed && { opacity: 0.85 }]}
      ...
      <Modal visible={modalOpen} animationType="slide" transparent onRequestClose={() => setModalOpen(false)}>
        <Pressable style={s.overlay} onPress={() => setModalOpen(false)}>
          <Pressable style={[s.sheet, { paddingBottom: insets.bottom + 20 }]} onPress={(e) => e.stopPropagation()}>
            ...
            {method && (
              <>
                <View style={[s.inputWrap, amountError ? s.inputError : null]}>
                  <TextInput
                    style={s.input}
                    placeholder="Amount"
                ...
                <View style={[s.inputWrap, addressError ? s.inputError : addressOk && address ? s.inputOk : null]}>
                  ...
                  {address ? (
                    <Ionicons
                      name={addressOk ? "checkmark-circle" : "close-circle"}
                      size={18}
                      color={addressOk ? "#4CAF50" : "#ef4444"}
                    />
                  ) : null}
                </View>
